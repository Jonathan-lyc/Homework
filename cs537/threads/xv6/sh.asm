...
nulterminate(struct cmd *cmd)
{
  ...
  switch(cmd->type){
      ...
      22:	ff 24 85 98 13 00 00 	jmp    *0x1398(,%eax,4)
...
peek(char **ps, char *es, char *toks)
{
  ...
  while(s < es && strchr(whitespace, *s))
      ...
      d2:	c7 04 24 98 14 00 00 	movl   $0x1498,(%esp)
     ...
     150:	83 c3 01             	add    $0x1,%ebx
{
  ...
  while(s < es && strchr(whitespace, *s))
     ...
     15a:	c7 04 24 98 14 00 00 	movl   $0x1498,(%esp)
  ...
  while(s < es && strchr(whitespace, *s))
     ...
     1aa:	c7 04 24 98 14 00 00 	movl   $0x1498,(%esp)
  ...
  *ps = s;
  ...
  default:
    ...
    while(s < es && !strchr(whitespace, *s) && !strchr(symbols, *s))
     ...
     1f3:	c7 04 24 9e 14 00 00 	movl   $0x149e,(%esp)
  ...
  default:
    ...
    while(s < es && !strchr(whitespace, *s) && !strchr(symbols, *s))
     ...
     215:	c7 04 24 98 14 00 00 	movl   $0x1498,(%esp)
...
panic(char *s)
{
  ...
  printf(2, "%s\n", s);
     ...
     419:	c7 44 24 04 31 14 00 	movl   $0x1431,0x4(%esp)
     420:	00 
...
parseredirs(struct cmd *cmd, char **ps, char *es)
{
  ...
  while(peek(ps, es, "<>")){
     450:	c7 44 24 08 e5 13 00 	movl   $0x13e5,0x8(%esp)
     457:	00 
     ...
     48c:	89 3c 24             	mov    %edi,(%esp)
{
  ...
  while(peek(ps, es, "<>")){
    ...
    if(gettoken(ps, es, &q, &eq) != 'a')
     ...
     4a9:	c7 04 24 c8 13 00 00 	movl   $0x13c8,(%esp)
...
parseexec(char **ps, char *es)
{
  ...
  if(peek(ps, es, "("))
     52f:	c7 44 24 08 e8 13 00 	movl   $0x13e8,0x8(%esp)
     536:	00 
  ...
  ret = parseredirs(ret, ps, es);
  while(!peek(ps, es, "|)&;")){
     586:	c7 44 24 08 ff 13 00 	movl   $0x13ff,0x8(%esp)
     58d:	00 
    ...
    if(tok != 'a')
     ...
     5bf:	74 0c                	je     5cd <parseexec+0xad>
      panic("syntax");
     5c1:	c7 04 24 ea 13 00 00 	movl   $0x13ea,(%esp)
    ...
    if(argc >= MAXARGS)
     ...
     5e4:	7e 8a                	jle    570 <parseexec+0x50>
      panic("too many args");
     5e6:	c7 04 24 f1 13 00 00 	movl   $0x13f1,(%esp)
...
parsepipe(char **ps, char *es)
{
  ...
  if(peek(ps, es, "|")){
     661:	c7 44 24 08 04 14 00 	movl   $0x1404,0x8(%esp)
     668:	00 
...
parseline(char **ps, char *es)
{
  ...
  cmd = parsepipe(ps, es);
  while(peek(ps, es, "&")){
     716:	c7 44 24 08 06 14 00 	movl   $0x1406,0x8(%esp)
     71d:	00 
  ...
  if(peek(ps, es, ";")){
     72e:	c7 44 24 08 02 14 00 	movl   $0x1402,0x8(%esp)
     735:	00 
...
parseblock(char **ps, char *es)
{
  ...
  if(!peek(ps, es, "("))
     7a5:	c7 44 24 08 e8 13 00 	movl   $0x13e8,0x8(%esp)
     7ac:	00 
  ...
  if(!peek(ps, es, ")"))
     7e9:	c7 44 24 08 24 14 00 	movl   $0x1424,0x8(%esp)
     7f0:	00 
  ...
  if(!peek(ps, es, ")"))
    ...
    panic("syntax - missing )");
     803:	c7 04 24 13 14 00 00 	movl   $0x1413,(%esp)
...
parseblock(char **ps, char *es)
{
  ...
  if(!peek(ps, es, "("))
    panic("parseblock");
     848:	c7 04 24 08 14 00 00 	movl   $0x1408,(%esp)
...
parsecmd(char *s)
{
  ...
  peek(&s, es, "");
     884:	c7 44 24 08 53 14 00 	movl   $0x1453,0x8(%esp)
     88b:	00 
     88c:	89 5c 24 04          	mov    %ebx,0x4(%esp)
{
  ...
  if(s != es){
    ...
    printf(2, "leftovers: %s\n", s);
     ...
     8a8:	c7 44 24 04 26 14 00 	movl   $0x1426,0x4(%esp)
     8af:	00 
     8b0:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
     8b7:	e8 04 07 00 00       	call   fc0 <printf>
    panic("syntax");
     8bc:	c7 04 24 ea 13 00 00 	movl   $0x13ea,(%esp)
     ...
     8f2:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
{
  ...
  if(pid == -1)
    panic("fork");
     8f8:	c7 04 24 35 14 00 00 	movl   $0x1435,(%esp)
...
getcmd(char *buf, int nbuf)
{
  ...
  printf(2, "$ ");
     922:	c7 44 24 04 3a 14 00 	movl   $0x143a,0x4(%esp)
     929:	00 
...
runcmd(struct cmd *cmd)
{
  ...
  default:
    panic("runcmd");
     983:	c7 04 24 3d 14 00 00 	movl   $0x143d,(%esp)
  ...
  case EXEC:
    ...
    printf(2, "exec %s failed\n", ecmd->argv[0]);
     ...
     9a8:	c7 44 24 04 44 14 00 	movl   $0x1444,0x4(%esp)
     9af:	00 
  ...
  switch(cmd->type){
     ...
     9ca:	ff 24 85 b0 13 00 00 	jmp    *0x13b0(,%eax,4)
  ...
  case REDIR:
    ...
    if(open(rcmd->file, rcmd->mode) < 0){
     ...
     b33:	c7 44 24 04 54 14 00 	movl   $0x1454,0x4(%esp)
     b3a:	00 
  ...
  case PIPE:
    ...
    if(pipe(p) < 0)
      panic("pipe");
     b50:	c7 04 24 64 14 00 00 	movl   $0x1464,(%esp)
...
main(void)
{
  ...
  while((fd = open("console", O_RDWR)) >= 0){
     ...
     b87:	00 
     b88:	c7 04 24 69 14 00 00 	movl   $0x1469,(%esp)
    ...
    if(fork1() == 0)
      runcmd(parsecmd(buf));
     ba7:	c7 04 24 c0 14 00 00 	movl   $0x14c0,(%esp)
  ...
  while(getcmd(buf, sizeof(buf)) >= 0){
     ...
     bc7:	00 
     bc8:	c7 04 24 c0 14 00 00 	movl   $0x14c0,(%esp)
     bcf:	e8 3c fd ff ff       	call   910 <getcmd>
     bd4:	85 c0                	test   %eax,%eax
     bd6:	78 70                	js     c48 <main+0xd8>
    if(buf[0] == 'c' && buf[1] == 'd' && buf[2] == ' '){
     bd8:	80 3d c0 14 00 00 63 	cmpb   $0x63,0x14c0
     bdf:	75 09                	jne    bea <main+0x7a>
     be1:	80 3d c1 14 00 00 64 	cmpb   $0x64,0x14c1
  ...
  while(getcmd(buf, sizeof(buf)) >= 0){
    if(buf[0] == 'c' && buf[1] == 'd' && buf[2] == ' '){
     bf8:	80 3d c2 14 00 00 20 	cmpb   $0x20,0x14c2
     bff:	90                   	nop
     c00:	75 e8                	jne    bea <main+0x7a>
      // Clumsy but will have to do for now.
      // Chdir has no effect on the parent if run in the child.
      buf[strlen(buf)-1] = 0;  // chop \n
     c02:	c7 04 24 c0 14 00 00 	movl   $0x14c0,(%esp)
     c09:	e8 c2 00 00 00       	call   cd0 <strlen>
      if(chdir(buf+3) < 0)
     c0e:	c7 04 24 c3 14 00 00 	movl   $0x14c3,(%esp)
  // Read and run input commands.
  while(getcmd(buf, sizeof(buf)) >= 0){
    if(buf[0] == 'c' && buf[1] == 'd' && buf[2] == ' '){
      // Clumsy but will have to do for now.
      // Chdir has no effect on the parent if run in the child.
      buf[strlen(buf)-1] = 0;  // chop \n
     c15:	c6 80 bf 14 00 00 00 	movb   $0x0,0x14bf(%eax)
      if(chdir(buf+3) < 0)
     c1c:	e8 c7 02 00 00       	call   ee8 <chdir>
     c21:	85 c0                	test   %eax,%eax
     c23:	79 9b                	jns    bc0 <main+0x50>
        printf(2, "cannot cd %s\n", buf+3);
     c25:	c7 44 24 08 c3 14 00 	movl   $0x14c3,0x8(%esp)
     c2c:	00 
     c2d:	c7 44 24 04 71 14 00 	movl   $0x1471,0x4(%esp)
     c34:	00 
...
printint(int fd, int xx, int base, int sgn)
{
  ...
  do{
    buf[i++] = digits[x % base];
     ...
     f4a:	f7 f7                	div    %edi
     f4c:	0f b6 92 86 14 00 00 	movzbl 0x1486(%edx),%edx
...
putc(int fd, char c)
{
  write(fd, &c, 1);
    ...
    10f8:	8b 55 d4             	mov    -0x2c(%ebp),%edx
        ap++;
        if(s == 0)
    10fb:	b9 7f 14 00 00       	mov    $0x147f,%ecx
        ap++;
      } else if(c == 'x' || c == 'p'){
        printint(fd, *ap, 16, 0);
        ap++;
...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    11c1:	a1 2c 15 00 00       	mov    0x152c,%eax
static Header base;
static Header *freep;
...
free(void *ap)
{
  ...
  freep = p;
    1211:	a3 2c 15 00 00       	mov    %eax,0x152c
}
    1216:	5b                   	pop    %ebx
    1217:	5e                   	pop    %esi
    1218:	5f                   	pop    %edi
  ...
  if(p + p->s.size == bp){
    ...
    p->s.ptr = bp;
  freep = p;
    123d:	a3 2c 15 00 00       	mov    %eax,0x152c
}
    1242:	5b                   	pop    %ebx
    1243:	5e                   	pop    %esi
    1244:	5f                   	pop    %edi
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    125c:	8b 0d 2c 15 00 00    	mov    0x152c,%ecx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
...
morecore(uint nu)
{
  ...
  if(nu < 4096)
    ...
    if(p == freep)
    1290:	3b 05 2c 15 00 00    	cmp    0x152c,%eax
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    12af:	89 58 04             	mov    %ebx,0x4(%eax)
      ...
      freep = prevp;
    12b2:	89 0d 2c 15 00 00    	mov    %ecx,0x152c
      return (void*)(p + 1);
...
morecore(uint nu)
{
  ...
  return freep;
    12f7:	8b 0d 2c 15 00 00    	mov    0x152c,%ecx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    130e:	c7 05 2c 15 00 00 24 	movl   $0x1524,0x152c
    1315:	15 00 00 
    base.s.size = 0;
    1318:	b9 24 15 00 00       	mov    $0x1524,%ecx
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    131d:	c7 05 24 15 00 00 24 	movl   $0x1524,0x1524
    1324:	15 00 00 
    base.s.size = 0;
    1327:	c7 05 28 15 00 00 00 	movl   $0x0,0x1528
    132e:	00 00 00 
...
lock_init(struct lock_t *lock)
{
  ...
  lock->locked = 0;
    ...
    1346:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
}
    134c:	5d                   	pop    %ebp
    134d:	c3                   	ret    
...
xchg(volatile uint *addr, uint newval)
{
  ...
  asm volatile("lock; xchgl %0, %1" :
    1351:	b9 01 00 00 00       	mov    $0x1,%ecx
...
lock_release(struct lock_t *lock)
{
    ...
    1378:	f0 87 02             	lock xchg %eax,(%edx)
